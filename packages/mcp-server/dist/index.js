...
var RaceAgent = class {
  ...
  async runTimingProbe(scope, findings) {
    ...
    for (const path of paths) {
      ...
      const fakeEmail = `does-not-exist-${randomUUID4().substring(0, 8)}@fracta.test`;
...
server.setRequestHandler(ListToolsRequestSchema, async () => ({
  tools: [
    ...
    {
      name: "scan_repo",
      description: "Auditoria SAST do REPOSIT\xD3RIO local, SEM config (depend\xEAncias, secrets, SAST stack-aware, LGPD/compliance, docs). Read-only. Ideal para auditar o projeto que voc\xEA est\xE1 desenvolvendo, na IDE.",
      inputSchema: {
        type: "object",
        properties: {
