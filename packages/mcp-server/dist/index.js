...
function parseIPv6(input) {
  ...
  if (m && m.index !== void 0) {
    ...
    v4groups = [v4 >>> 16 & 65535, v4 & 65535];
    str = str.slice(0, m.index);
  ...
  const bytes = [];
  for (const g of full) bytes.push(g >>> 8 & 255, g & 255);
  return bytes;
}
...
function isBlockedV6Bytes(b) {
  ...
  const embeddedV4 = (i) => (b[i] << 24 | b[i + 1] << 16 | b[i + 2] << 8 | b[i + 3]) >>> 0;
  if (allZero(b, 0, 10) && (b[10] === 255 && b[11] === 255 || b[10] === 0 && b[11] === 0)) {
    return isBlockedV4Int(embeddedV4(12));
  }
  if (b[0] === 0 && b[1] === 100 && b[2] === 255 && b[3] === 155 && allZero(b, 4, 12)) {
    return isBlockedV4Int(embeddedV4(12));
  }
  if (b[0] === 32 && b[1] === 2) {
...
var defaultResolver2 = async (host) => {
...
async function validateScanUrl(input, opts = {}) {
  const resolve = opts.resolve ?? defaultResolver2;
  const raw = /^[a-z][a-z0-9+.-]*:\/\//i.test(input) ? input : `https://${input}`;
  let url;
  try {
    url = new URL(raw);
  } catch {
    throw new SsrfError(`URL inv\xE1lida: ${input}`);
  }
  if (url.protocol !== "http:" && url.protocol !== "https:") {
    throw new SsrfError(`Esquema n\xE3o permitido: ${url.protocol} (use http/https)`);
  }
  if (url.username || url.password) {
    throw new SsrfError("Credenciais embutidas na URL n\xE3o s\xE3o permitidas");
  }
  if (url.port && BLOCKED_PORTS.has(Number(url.port))) {
    throw new SsrfError(`Porta n\xE3o permitida: ${url.port} (servi\xE7o interno)`);
  }
  ...
  if (isBlockedIp(host)) {
    throw new SsrfError("Endere\xE7o interno/privado n\xE3o \xE9 escane\xE1vel");
  }
  let addrs;
  try {
    addrs = await resolve(host);
  } catch {
    throw new SsrfError(`N\xE3o foi poss\xEDvel resolver o host: ${host}`);
  }
  if (addrs.length === 0) {
    throw new SsrfError(`Host sem endere\xE7o resolv\xEDvel: ${host}`);
  }
  if (addrs.some(isBlockedIp)) {
    throw new SsrfError("O host resolve para um endere\xE7o interno/privado \u2014 recusado");
  }
  return url;
}
function makeValidatingLookup(allowPrivate) {
  return (hostname, options, callback) => {
    dnsLookup(hostname, options, (err, address, family) => {
      ...
      if (!allowPrivate && list.some(isBlockedIp)) {
        return callback(
          Object.assign(new Error(`SSRF: conex\xE3o recusada para ${hostname} \u2192 ${list.join(", ")}`), { code: "ESSRFBLOCKED" }),
          void 0
        );
      }
...
function createSafeDispatcher(allowPrivate = false) {
  return new Agent({
    connect: { lookup: makeValidatingLookup(allowPrivate), timeout: 8e3 },
    headersTimeout: 1e4,
    bodyTimeout: 1e4
  });
}
function createSafeClient(allowPrivate = false) {
  const dispatcher = createSafeDispatcher(allowPrivate);
  return (url) => new FractaHttpClient(url, {}, { dispatcher, redirect: "follow" });
}
function findCookieIssues(setCookies, saas, runId) {
  const out = [];
  for (const raw of setCookies) {
    ...
    out.push({
      ...
      camada: "security",
      severity: "low",
      title: `Cookie sem flags de seguran\xE7a: ${name}`,
      description: `O cookie "${name}" n\xE3o define: ${missing.join(", ")}. Sem essas flags ele \xE9 mais exposto a roubo (XSS) e envio cross-site (CSRF).`,
      evidence: `Set-Cookie: ${name}=\u2026 (faltam: ${missing.join(", ")})`,
      recommendation: "Defina Secure, HttpOnly e SameSite (Lax/Strict) nos cookies de sess\xE3o.",
      proposedFix: {
        description: `Recrie o cookie "${name}" com as flags que faltam. No header:  Set-Cookie: ${name}=<valor>; Secure; HttpOnly; SameSite=Lax; Path=/ .  Express:  res.cookie('${name}', v, { secure: true, httpOnly: true, sameSite: 'lax' }) .  Next.js (route handler):  cookies().set('${name}', v, { secure: true, httpOnly: true, sameSite: 'lax', path: '/' }) .`,
        riskOfApplying: "PROPOSTA \u2014 n\xE3o aplicada. HttpOnly impede leitura via JS (correto p/ cookie de sess\xE3o; quebra s\xF3 se algum front l\xEA esse cookie por JS). SameSite=Strict pode quebrar login via redirect cross-site \u2014 use Lax nesses casos."
      },
...
var TRACKING_COOKIE = /^(_ga|_gid|_gat|_gcl|_fbp|_fbc|_hj|_clck|_clsk|_tt_|_uet|IDE|MUID|personalization_id)/i;
var ANCHOR_RE = /<a\b[^>]*href=["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>/gi;
function findPrivacyHref(html) {
  if (!html) return null;
  const candidates = [];
...
var DISCLOSURES = [
  ...
  { label: "reten\xE7\xE3o/elimina\xE7\xE3o de dados", re: /reten[çc][ãa]o|por quanto tempo|prazo.{0,30}(armazena|conserva|guard)|art(igo)?\.?\s*1[56]\b/i },
  { label: "transfer\xEAncia internacional (Art. 33)", re: /transfer[êe]ncia internacional|art(igo)?\.?\s*33|cl[áa]usulas?[- ]?padr[ãa]o|standard contractual|fora do (pa[íi]s|Brasil)/i }
];
function lgpdFinding(saas, runId, rule, severity, title, description, recommendation, evidence) {
  return {
...
function checkLgpdLite(html, setCookies, saas, runId, policy, botChallenge) {
  if (botChallenge) {
    return [lgpdFinding(
      saas,
      runId,
      "lgpd-content-blocked",
      "info",
      `Conte\xFAdo protegido por bot-management (${botChallenge.vendor}) \u2014 leitura passiva limitada`,
      `N\xE3o consegui ler o conte\xFAdo da p\xE1gina automaticamente: o ${botChallenge.vendor} respondeu ao scan com um desafio de bot. Isso \xE9 um SINAL DE SEGURAN\xC7A do seu site, n\xE3o uma falha. Consequ\xEAncia honesta: passivamente n\xE3o d\xE1 pra verificar Pol\xEDtica de Privacidade, rastreadores nem cookies \u2014 e os headers/TLS refletem a borda do ${botChallenge.vendor}, n\xE3o necessariamente a sua origem.`,
      `Para um diagn\xF3stico do que est\xE1 atr\xE1s da prote\xE7\xE3o, rode a an\xE1lise autenticada (CLI). Se quiser um scan passivo mais fiel, libere o "FractaBot" no seu bot-management.`
    )];
  }
  const out = [];
  const h = html || "";
  if (!PRIVACY_HINT.test(h)) {
    out.push(lgpdFinding(
      saas,
      runId,
      "lgpd-no-privacy-link",
      "low",
      "Sem link vis\xEDvel de Pol\xEDtica de Privacidade (LGPD-lite, beta)",
      'Heur\xEDstica beta: n\xE3o encontrei men\xE7\xE3o a "pol\xEDtica de privacidade" na p\xE1gina. A LGPD exige transpar\xEAncia sobre o tratamento de dados (Art. 9\xBA). Pode ser falso-positivo (link em outra p\xE1gina).',
      "Publique e linke uma Pol\xEDtica de Privacidade clara, com base legal e contato do encarregado (DPO)."
    ));
  }
  ...
  const trackingCookies = setCookies.map((c) => c.split("=")[0]?.trim() ?? "").filter((name) => name && TRACKING_COOKIE.test(name));
  if (trackingCookies.length > 0) {
    out.push(lgpdFinding(
      saas,
      runId,
      "lgpd-tracking-cookie-precons",
      "low",
      "Cookie de rastreamento no primeiro acesso (LGPD-lite, beta)",
      `O(s) cookie(s) ${trackingCookies.join(", ")} \u2014 de rastreamento \u2014 foram definidos j\xE1 no primeiro acesso, antes de qualquer intera\xE7\xE3o ou consentimento. A LGPD exige consentimento PR\xC9VIO para cookies n\xE3o-essenciais (Art. 8\xBA).`,
      "S\xF3 defina cookies n\xE3o-essenciais ap\xF3s o consentimento expl\xEDcito do usu\xE1rio.",
  ...
  if (policy && policy.text.trim().length > 200) {
    ...
    const desc = missing.length ? `Li automaticamente a sua Pol\xEDtica de Privacidade. Ela MENCIONA: ${covered.join("; ") || "\u2014"}. N\xC3O encontrei men\xE7\xE3o clara a: ${missing.join("; ")}. An\xE1lise determin\xEDstica por palavras-chave (Art. 9\xBA/18/33/41), sem IA \u2014 pode ser falso-positivo se a sua pol\xEDtica usa outra reda\xE7\xE3o.` : `Li automaticamente a sua Pol\xEDtica de Privacidade: ela menciona todos os itens-chave que verifico \u2014 ${covered.join("; ")}. An\xE1lise determin\xEDstica por palavras-chave; n\xE3o substitui revis\xE3o jur\xEDdica.`;
    out.push(lgpdFinding(
      saas,
      runId,
      "lgpd-policy-analysis",
      "info",
      "Pol\xEDtica de Privacidade analisada automaticamente (LGPD-lite, beta)",
      desc,
      missing.length ? "Revise a pol\xEDtica para cobrir explicitamente os itens n\xE3o encontrados, conforme os artigos da LGPD." : "Mantenha a pol\xEDtica coerente com o tratamento real de dados.",
      `pol\xEDtica lida: ${policy.url}`
    ));
  }
  return out;
}
var CF_BODY = /just a moment\.\.\.|challenge-platform|cf-browser-verification|cf_chl_opt|_cf_chl|enable javascript and cookies to continue|checking (if the site connection is secure|your browser before)/i;
function detectBotChallenge(status, headers, body) {
  const get = (k) => {
    const v = headers[k] ?? headers[k.toLowerCase()];
    return (Array.isArray(v) ? v.join(" ") : v ?? "").toString().toLowerCase();
  };
  const server2 = get("server");
  const b = body || "";
  if (get("cf-mitigated")) return { challenged: true, vendor: "Cloudflare" };
  if (CF_BODY.test(b) && (server2.includes("cloudflare") || /cf-chl|cloudflare/i.test(b))) {
    return { challenged: true, vendor: "Cloudflare" };
  }
  if (server2.includes("cloudflare") && (status === 403 || status === 503 || status === 429) && CF_BODY.test(b)) {
    return { challenged: true, vendor: "Cloudflare" };
  }
  if (/incapsula|_incap_|visid_incap|imperva/i.test(b) || server2.includes("incapsula")) {
...
function grade(findings) {
  ...
  const grade2 = score >= 90 ? "A" : score >= 75 ? "B" : score >= 60 ? "C" : score >= 40 ? "D" : score >= 20 ? "E" : "F";
  return { grade: grade2, score };
}
function stripHtml(html) {
  return html.replace(/<script\b[^>]*>[\s\S]*?<\/script>/gi, " ").replace(/<style\b[^>]*>[\s\S]*?<\/style>/gi, " ").replace(/<[^>]+>/g, " ").replace(/&nbsp;/gi, " ").replace(/\s+/g, " ").trim();
}
var PassiveScanner = class {
  constructor(opts = {}) {
    this.opts = opts;
    if (opts.allowPrivateForTest && process.env.NODE_ENV !== "test") {
      throw new Error("allowPrivateForTest s\xF3 \xE9 permitido em ambiente de teste (NODE_ENV=test)");
    }
  }
  opts;
  async scan(input) {
    const allowPrivate = !!this.opts.allowPrivateForTest;
    const url = allowPrivate ? new URL(/^https?:\/\//.test(input) ? input : `https://${input}`) : await validateScanUrl(input);
    ...
    try {
      ...
      const res = await client.request("/", { timeoutMs: 8e3 });
      contentRan = true;
      const html = res.raw ?? "";
      detectedStack = detectStack(res.headers, html);
      ...
      if (challenge.challenged && challenge.vendor) {
        findings.push(...checkLgpdLite(html, setCookie, saas, runId, void 0, { vendor: challenge.vendor }));
      } else {
        ...
        if (href) {
          try {
            ...
            if (policyUrl.host === url.host && /^https?:$/.test(policyUrl.protocol)) {
              const pres = await client.request(policyUrl.pathname + policyUrl.search, { timeoutMs: 8e3 });
    ...
    const verdict = graded ? "ok" : "inconclusive";
    const g = graded ? grade(findings) : { grade: null, score: null };
    return {
      url: url.toString(),
      findings,
      grade: g.grade,
...
var RaceAgent = class {
  ...
  async runTimingProbe(scope, findings) {
    ...
    for (const path of paths) {
      ...
      const fakeEmail = `does-not-exist-${randomUUID2().substring(0, 8)}@fracta.test`;
...
function loadSqlite() {
  try {
    return nodeRequire2("node:sqlite");
  } catch (err) {
    throw new Error(
...
function fmtFinding(f) {
  const low = f.confidence === "low" ? " \xB7 baixa confian\xE7a" : "";
  return `- [${f.severity}${low}] ${f.title}`;
}
function sastConfidenceSummary(findings) {
  const confirmed = findings.filter((f) => f.confidence !== "low");
  const n = (s) => confirmed.filter((f) => f.severity === s).length;
  const lowConf = findings.length - confirmed.length;
  const conf = `alta confian\xE7a: ${n("critical")} critical \xB7 ${n("high")} high \xB7 ${n("medium")} medium`;
  const low = lowConf > 0 ? ` \xB7 +${lowConf} de baixa confian\xE7a (contexto de teste/exemplo ou regra ampla do semgrep \u2014 revis\xE3o, n\xE3o reprovam)` : "";
  return conf + low;
}
...
// src/promo.ts
var MCP_FOOTER = "\n\n\u2014\nFracta \xE9 gr\xE1tis e open-source \xB7 [monitore isto a cada deploy](https://fracta.pro?ref=mcp&utm_source=fracta-mcp&utm_medium=mcp&utm_campaign=footer) (badge + regress\xE3o cont\xEDnua).\nDa PreviusIA \u2014 tamb\xE9m fazemos o [zap-api.tech](https://zap-api.tech?ref=fracta-mcp&utm_source=fracta-mcp&utm_medium=mcp&utm_campaign=crosssell): API de WhatsApp pra devs, segura por padr\xE3o.";
...
function hasNodeSqlite() {
  try {
    createRequire3(import.meta.url)("node:sqlite");
    return true;
  } catch {
    return false;
  }
...
async function loadTargets() {
  const raw = await readFile6(TARGETS_CONFIG, "utf-8");
...
function buildSastOrchestrator(repoPath) {
...
var server = new Server(
  { name: "fracta", version: package_default.version },
  { capabilities: { tools: {} } }
);
server.setRequestHandler(ListToolsRequestSchema, async () => ({
  tools: [
    {
      name: "passive_scan",
      description: "Scan PASSIVO de qualquer URL, SEM config (headers, TLS, cookies, LGPD-lite: l\xEA a pol\xEDtica e detecta bot-management). Zero intrus\xE3o \u2014 seguro para qualquer site. Retorna nota A\u2013F na hora.",
      inputSchema: {
        type: "object",
    ...
    {
      name: "scan_repo",
      description: "Auditoria SAST do REPOSIT\xD3RIO local, SEM config (depend\xEAncias, secrets, SAST stack-aware + SAST SEM\xC2NTICO via semgrep/dataflow quando instalado, LGPD/compliance, docs). Read-only. Ideal para auditar o projeto que voc\xEA est\xE1 desenvolvendo, na IDE. (semgrep ausente \u2192 skip honesto, n\xE3o falso-verde.)",
      inputSchema: {
        type: "object",
        properties: {
          path: { type: "string", description: "Caminho do reposit\xF3rio (default: diret\xF3rio atual)" },
    ...
    {
      name: "verify_consent",
      description: "Verifica\xE7\xE3o em RUNTIME (browser headless) do comportamento de consentimento: carrega a p\xE1gina, captura trackers/cookies que disparam ANTES de qualquer consentimento e detecta CMP real. Confirma com evid\xEAncia o que o passive_scan s\xF3 sup\xF5e. Funciona com o Google Chrome do sistema ou `npx playwright install chromium`; use em sites que voc\xEA autoriza. SSRF: bloqueia requests diretos a hosts internos, valida cada hop de redirect por-hop (302\u2192host interno \xE9 abortado antes de o browser alcan\xE7\xE1-lo) e recusa navega\xE7\xE3o que aterrisse em host privado \u2014 use ainda assim s\xF3 em alvos autorizados.",
      inputSchema: {
        type: "object",
    ...
    {
      name: "verify_csp",
      description: 'READ-ONLY, prova em RUNTIME (browser headless) a COBERTURA da CSP \u2014 n\xE3o s\xF3 o header. Carrega a p\xE1gina, captura os SecurityPolicyViolationEvent reais e mostra quantos <script> a pol\xEDtica de fato cobre. Pega o caso "CSP estrita e bonita, mas N scripts sem nonce" que um check de header jamais pegaria. Aceite uma URL direta, sem config. Precisa do Chrome do sistema ou `npx playwright install chromium`.',
      inputSchema: {
        type: "object",
    ...
    {
      name: "test_idor",
      description: "Testa IDOR: acesso direto por ID + enumera\xE7\xE3o. Se o target tiver `crossTenant` (2\xAA conta + `ownedResources`) no targets.yaml, PROVA isolamento multi-tenant com 2 contas reais (A tenta ler recurso de B \u2192 200 = vazamento CONFIRMADO). Intrusivo/autenticado: use em staging.",
      inputSchema: { type: "object", properties: { target: { type: "string" } }, required: ["target"] }
    },
    {
      name: "check_headers",
      description: "READ-ONLY (s\xF3 um GET dos response headers) \u2014 seguro em PRODU\xC7\xC3O, sem intrus\xE3o. Aceite uma URL direta (ex.: https://exemplo.com.br) SEM config nenhuma, ou o nome de um target do targets.yaml. Verifica security headers (CSP, HSTS, X-Frame-Options\u2026) e configura\xE7\xE3o de CORS.",
      inputSchema: {
        type: "object",
    ...
    },
    {
      ...
      description: "Gera relat\xF3rio do \xFAltimo scan em markdown, JSON ou SARIF 2.1.0 (para subir no GitHub Code Scanning e virar control de CI)",
      inputSchema: {
        type: "object",
        properties: {
          target: { type: "string" },
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    if (name === "passive_scan") {
      ...
      const head = r.verdict === "inconclusive" ? "INCONCLUSIVO (alvo n\xE3o avali\xE1vel \u2014 aus\xEAncia de achados \u2260 seguro)" : `Nota ${r.grade} (${r.score}/100)`;
      const bySev = r.findings.reduce((m, f) => (m[f.severity] = (m[f.severity] ?? 0) + 1, m), {});
      const lines = r.findings.map(fmtFinding);
      const txt = `Scan passivo de ${r.url}
${head} \xB7 checks: ${r.checks.map((c) => `${c.name}=${c.status}`).join(", ")}
Severidades: ${JSON.stringify(bySev)}

${lines.join("\n") || "(sem achados nos checks executados)"}`;
      return { content: [{ type: "text", text: txt + promoFooter() }] };
    }
    if (name === "scan_repo") {
      const path = a.path && a.path.trim() || process.cwd();
      if (!existsSync(path)) return { content: [{ type: "text", text: `Caminho n\xE3o encontrado: ${path}` }] };
      const rawStack = args?.stack;
      const stack = Array.isArray(rawStack) ? rawStack.map(String) : [];
      const target = { name: "repo-scan", url: "file://local", stack, repoPath: path };
      const report = await buildSastOrchestrator(path).scan(target);
      lastReports.set("repo-scan", report);
      const { mdPath } = await new FractaReporter({ toolVersion: package_default.version }).save(report);
...
${lines.join("\n")}${more}`;
    ...
    if (name === "scan_target" || name === "test_auth" || name === "test_idor" || name === "check_headers") {
      ...
      if (name === "check_headers" && looksLikeUrl(a.target)) {
        ...
        if (!target) {
          const hint = name === "check_headers" ? ` \u2014 ou passe uma URL http(s) direta (ex.: https://exemplo.com.br), sem config.` : "";
          return { content: [{ type: "text", text: `Target "${a.target}" n\xE3o encontrado no targets.yaml${hint}` }] };
        }
      }
      let agentFilter;
      ...
      const reporter = new FractaReporter({ toolVersion: package_default.version });
      ...
      const summary = `Scan conclu\xEDdo. Status: ${statusTxt}. ${sastConfidenceSummary(report.findings)}. Relat\xF3rio: ${mdPath}`;
      return { content: [{ type: "text", text: summary + promoFooter() }] };
    }
    if (name === "audit_docs") {
      ...
      return { content: [{ type: "text", text: `Docs audit: ${report.summary.total} findings. Medium: ${report.summary.medium}, Low: ${report.summary.low}` + promoFooter() }] };
    }
    ...
    if (name === "generate_report") {
      ...
      return { content: [{ type: "text", text: `Relat\xF3rio salvo em: ${path}` + promoFooter() }] };
    }
