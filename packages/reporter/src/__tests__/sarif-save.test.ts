...
describe('FractaReporter.save → SARIF', () => {
  it('emite um arquivo .sarif válido junto do md/json', async () => {
    ...
    // o markdown traz o scorecard OWASP (capstone de postura)
    const md = await readFile(r.mdPath, 'utf-8')
    expect(md).toMatch(/Postura por OWASP Top 10/)
    expect(md).toMatch(/A03 — Injection/)
